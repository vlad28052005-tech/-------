...
function parseHMToDate(hm, baseDate) { if (!hm) return null; const [h, m] = hm.split(':').map(Number); const d = new Date(baseDate);
...
function updateWeekLabel() { document.getElementById('weekLabel').textContent = isTopWeek(currentMonday) ? 'Верхній' : 'Нижній'; const start = new Date(currentMonday); const end = new Date(currentMonday.getTime() + 4 * 86400000);
...
function render(withStagger = true) {
    ...
    for (let i = 0; i < DAYS.length; i++) {
        ...
        if (pairs.length === 0) { const empty = document.createElement('div');
        ...
        if (viewingCurrentWeek && sameDay(dayDate, today)) {
            ...
            if (currentIdx !== -1) { const after = pairs.find(v => v.idx > currentIdx);
                nextIdx = after ? after.idx : -1; } else { const future = pairs.find(v => { const t = getTimeFor(dKey, v.idx); const st = parseHMToDate(t.start, now); return st && st > now; });
        ...
        for (const v of pairs) {
            ...
            if (box.classList.contains('next')) { const badge = document.createElement('div');
...
document.addEventListener('touchend', e => { if (!e.changedTouches) return; const dx = e.changedTouches[0].screenX - startX; const dy = e.changedTouches[0].screenY - startY; if (Math.abs(dx) < 60 || Math.abs(dy) > Math.abs(dx)) return; if (dx > 0) changeWeekAnimated(-1);
    else changeWeekAnimated(1); }, { passive: true });
...
function scheduleNextPreciseUpdate() {
    ...
    if (!todayKey) { const nd = new Date(now);
        nd.setDate(nd.getDate() + 1);
        nd.setHours(0, 1, 0, 0);
        nextUpdateTimer = setTimeout(() => render(false), nd - now); return; }
    ...
    if (candidates.length === 0) { const nd = new Date(now);
        nd.setDate(nd.getDate() + 1);
        nd.setHours(0, 1, 0, 0);
        candidates.push(nd); }
